...
app.post("/natal-chart", (req, res) => {
  ...
  const houses = se.swe_houses(jd, lat, lon, "P");
  const ascmc = se.swe_calc_ut(jd, se.SE_ASC, se.SEFLG_SWIEPH);

  if (!houses || !houses.house || !Array.isArray(houses.house)) {
    return res.status(500).json({ error: "Failed to calculate houses" });
  }
  ...
  res.json({
    ascendant: ascmc.longitude.toFixed(2),
    houses,
    planets,
    julianDay: jd
  });
